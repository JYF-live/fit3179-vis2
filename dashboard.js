...
let tempView, gustView, rainView, tempPieView, rainPieView, mapView, stackedView;
...
Promise.all([
    ...
    fetch('vega_gust_map.json').then(response => response.json()),
    fetch('stacked_area_chart_vega.json').then(response => response.json())
]).then(function([tempSpec, gustSpec, rainSpec, tempPieSpec, rainPieSpec, mapSpec, stackedSpec]) {
    
    // Embed the choropleth map
    vegaEmbed('#choroplethMap', mapSpec, {actions: false}).then(result => {
        mapView = result.view;
    });
    
    // Embed the stacked area chart
    vegaEmbed('#stackedAreaChart', stackedSpec, {actions: false}).then(result => {
        stackedView = result.view;
    });
